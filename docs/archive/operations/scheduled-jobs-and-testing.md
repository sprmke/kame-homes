...
| **Voice session reaper** | `voice-receptionist-reaper` | **Global** `POST` **`{}`**: closes stale connecting/active voice leases, records terminal usage once, and removes unverified caption evidence past the configured 1-90 day retention. Optional **`VOICE_RECEPTIONIST_REAPER_SECRET`** / header **`X-Voice-Receptionist-Reaper-Secret`**. Scheduled by `sync_voice_receptionist_reaper_cron_job()` every minute. Session-end RPC locking and conditional usage claims make overlap safe. |
| **Voice provider canary** | `voice-receptionist-canary` | **Global** `POST` **`{}`**: weekly non-production token-mint and `setupComplete` probe with no audio. Persists model/protocol and mint/setup latency in global AI settings; production returns a successful skip. Requires matching Edge **`VOICE_RECEPTIONIST_CANARY_SECRET`** and Vault **`voice_receptionist_canary_secret`** values. Scheduled by `sync_voice_receptionist_canary_cron_job()` as `voice-receptionist-canary-weekly` at **`0 2 * * 1`** UTC. |
...
| `voice-receptionist-reaper` | minute | **Yes** — terminal transition is row-locked and idempotent; usage recording is conditionally claimed; transcript retention deletes only expired rows. |
