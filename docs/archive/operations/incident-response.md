...
| **AI provider outage (Gemini/Groq)**   | The AI plan's quota/credit/upgrade-hook layer already degrades gracefully. For voice, set `voice_receptionist_rollout_percentage` to `0`, clear its property allowlist, or remove `voice_receptionist` from `allowed_features`; the guest keeps text chat and **Message host** handoff. Check `voice-receptionist-canary` in staging, provider close/error rates, and `voice_receptionist_sessions.failure_code`. Core booking remains independent of AI.                                            |
...
### Voice receptionist ownership and rollback triggers

The super-admin on call owns the platform voice kill switch and staged rollout. Set voice rollout
to zero immediately when the staging canary fails twice, provider errors exceed 5% for 15 minutes,
clean/recovered completion falls below 99%, p95 first audio exceeds 5 seconds, or a transcript/data
boundary incident is suspected. Property operators may disable their own opt-in but must not be the
only incident control. Voice failure must leave guest text chat, **Message host**, booking, and
checkout available.

## Cron disable/enable during a restore

19 functions run via hosted `pg_cron` + `pg_net` (not `config.toml` schedules): `activity-log-retention-cron`, `analytics-ai-review-cron`, `contract-expiry-cron`, `dashboard-assistant-expire-pending-actions`, `expire-parking-broadcasts`, `marketing-generation-sweeper`, `meta-inbox-webhook-healthcheck`, `platform-billing-cron`, `property-page-views-prune-cron`, `query-cache-sweep-cron`, `send-parking-reminders`, `smart-pricing-cron`, `telegram-admin-cron`, `telegram-finance-cron`, `telegram-maintenance-cron`, `telegram-marketing-cron`, `telegram-staff-cron`, `voice-receptionist-canary`, `voice-receptionist-reaper`.
