...
- Q: Why did a guest message stay pending even though Send automatically is on?
  A: Auto-reply only sends answers it can back up with your property details. If the draft mentions a price, amount, account number or another guest that it cannot confirm, or the AI could not produce an answer, nothing is sent and the thread stays pending for you. It also waits about 90 seconds between replies and sends at most 10 AI replies per conversation per hour.
- Q: Is there a length limit on messages?
  A: Yes, 4,000 characters per message, for both you and the guest.

**Auto-reply safety (`_shared/inboxAutoReplyPolicy.ts`):** a draft the reply guard flags, or the canned fallback line, is **not sent**; `reply_status` stays `pending` so the host sees it. Per conversation: 90 s cooldown, max 10 AI replies/hour. Meta webhook and web chat run the AI step after acknowledging the inbound message. **Suggest** is scoped: a conversation outside the caller's property/parking scope returns 404. Message bodies are capped at 4,000 chars (composer `maxLength`, server check). See [`ai-platform.md`](../../../architecture/ai-platform.md) §5.
