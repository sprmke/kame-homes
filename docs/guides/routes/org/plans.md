...
- **Layout:** current-plan summary banner (Free when there is no paid org subscription, otherwise the active tier), then tabs. On phone, banner actions stack full-width with a short **Manage** label (full **Manage subscription** from `sm` up). Tabs: **Billing** (first) / Plans / Compare.
...
| Billing tab           | `ui/src/features/dashboard/plans/components/PlanBillingPanel.tsx`, `ui/src/features/dashboard/plans/components/OrgPlanTransactions.tsx`, `ui/src/features/dashboard/plans/components/PlanCheckoutConfirmationBanner.tsx`                                                 |
