...
## Still needed from the user before Phase 1 starts

Not blockers for finalizing the plan; blockers for good execution.

1. **10 real host prompts** for the evaluation corpus. Synthetic prompts alone will
   flatter the enhancer, because I would be writing both the test and the answer.
2. **One fully-populated test property** — `name`, `city`, `type`, `residence_name`,
   `max_guests`, amenities, `brand_color`. Phase 2 cannot be verified against a sparse
   row; context injection would look like it works while injecting nothing.
3. **Examples of bad output**, if any were kept. This plan targets a failure mode I
   inferred from reading the code, not one I observed. If the real complaint is something
   else (wrong style, warped rooms, unreadable text), Phase 1's system instruction should
   be aimed at that specifically.
4. **Confirmation of the ship gate** (mean postability +1.0) once the first pairs exist —
   the threshold is a reasonable starting number, not a measured one.
