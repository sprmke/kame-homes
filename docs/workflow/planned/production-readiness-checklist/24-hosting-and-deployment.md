...
| 2   | Rehearse rollback on hosted-dev with a measured RTO (24.2). **Partial:** runbook + preflight (`2026-09-21`: linked `fwor…`, dev backups on disk; `rollback --dry-run` needs `DEV_DB_URL`). Measured drill still open.                                                                                                                                                                                  | Hosted-dev operator |
| 3   | Prod path: manual approval with the schema diff attached (24.4). `kamewave` stays the unlock.                                                                                                                                                                                                                                                                                                          | CI + policy         |
| 4   | Smoke tests: authenticated reads, a cleaned-up write, asset headers, PWA version, cron presence (24.5). **Partial (2026-09-21):** cd-dev [`35569769373`](https://github.com/sprmke/kame-homes/actions/runs/35569769373) — cron probe + partial listing smoke + transport compression; `verify:deployed-preview` incl. `/pwa-version.json`; adversarial **15/15**. Authenticated read/write still open. | CI + hosted         |
...
- `adversarialAuthLive.test.ts` — **15/15** on cd-dev run [`35569769373`](https://github.com/sprmke/kame-homes/actions/runs/35569769373) (2026-09-21).
