...
| 4   | Smoke tests: authenticated reads, a cleaned-up write, asset headers, PWA version, cron presence (24.5). **Partial (2026-09-21):** cd-dev [`35556921767`](https://github.com/sprmke/kame-homes/actions/runs/35556921767) — cron probe + partial listing smoke + transport compression; `verify:deployed-preview` incl. `/pwa-version.json`; adversarial **8/8**. Authenticated read/write still open. | CI + hosted         |
