...
| 4   | Smoke tests: authenticated reads, a cleaned-up write, asset headers, PWA version, cron presence (24.5). **Partial (2026-09-21):** `ci-smoke.sh` asserts `activity-log-retention-cron` in functions list + transport compression; `verify:deployed-preview` asserts `/pwa-version.json` kill-switch JSON + cache headers on `dev.kamehomes.space`; adversarial **8** cases (pending cd-dev after push). Authenticated read/write still open. | CI + hosted         |
...
- `adversarialAuthLive.test.ts` — **6/6** on cd-dev run `35555310204`; file expanded to **8** cases (hosted dev **8/8** locally, 2026-09-21).
