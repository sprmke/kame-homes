...
| 7   | Adversarial suite (21.6 table). **Partial:** `adversarialAuthLive.test.ts` (**8 cases**) **8/8** on hosted dev + **cd-dev** run [`35556921767`](https://github.com/sprmke/kame-homes/actions/runs/35556921767) (2026-09-21); full role×endpoint matrix + Playwright still open.     | Tests + seed data |
...
- [ ] Adversarial suite green; each case fails correctly when the check is removed. **Partial:** live smoke (8 cases in `adversarialAuthLive.test.ts`) **8/8** on hosted dev + cd-dev `35556921767` (2026-09-21).
