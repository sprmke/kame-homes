...
| 7   | Adversarial suite (21.6 table). **Partial:** `adversarialAuthLive.test.ts` (**8 cases**: `serveAuthenticated`/`serveAdmin`/`serveSuperAdmin`, two `serveCronPost`, malformed JWT) **8/8** on hosted dev (2026-09-21, local); **cd-dev** still **6/6** until next `supabase/**` deploy; full role×endpoint matrix + Playwright still open. | Tests + seed data |
...
- [ ] Adversarial suite green; each case fails correctly when the check is removed. **Partial:** live smoke (8 cases in `adversarialAuthLive.test.ts`) **8/8** on hosted dev (2026-09-21); cd-dev re-run pending next deploy.
