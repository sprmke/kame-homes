...
| 2   | Client/server parity test for every role × endpoint (21.2).                                                                                                                                                                                                                          | Tests + seed data |
| 3   | Sign-out on a shared device: query cache / realtime / PWA Cache Storage. **Code path exists** (`PwaQueryPersistence` clears TanStack Query + `purgeOfflineState` on `SIGNED_OUT`; admin/guest `signOut` calls `supabase.auth.signOut()`). Not proven on a real device.               | Display + PWA     |
...
| 7   | Adversarial suite (21.6 table). **Partial:** `adversarialAuthLive.test.ts` (**6 cases**: authenticated/admin/super-admin/cron + malformed JWT) green on hosted dev and **cd-dev** runs `35544063765`, `35545944443` (2026-09-21); full role×endpoint matrix + Playwright still open. | Tests + seed data |
...
- [ ] Adversarial suite green; each case fails correctly when the check is removed. **Partial:** live smoke (6 cases in `adversarialAuthLive.test.ts`: `serveAuthenticated`/`serveAdmin`/`serveSuperAdmin`/`serveCronPost` + malformed JWT) on hosted dev + cd-dev (2026-09-21).
