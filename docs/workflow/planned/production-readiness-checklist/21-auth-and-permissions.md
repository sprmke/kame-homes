...
| 7   | Adversarial suite (21.6 table). **Partial:** `adversarialAuthLive.test.ts` (**12 cases**, incl. `get-health` positive control) **12/12** hosted dev (2026-09-21); cd-dev **11/11** on [`35558455879`](https://github.com/sprmke/kame-homes/actions/runs/35558455879) until next deploy. Full 21.6 table + Playwright still open. | Tests + seed data |
...
- [ ] Adversarial suite green; each case fails correctly when the check is removed. **Partial:** live smoke (**12** cases) **12/12** on hosted dev (2026-09-21); cd-dev **11/11** on `35558455879` until next deploy.
