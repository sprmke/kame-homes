...
| 7   | Adversarial suite (21.6 table). **Partial:** `adversarialAuthLive.test.ts` (**13 cases**, incl. `get-health` + `list-activity-log`; prior **12/12** on cd-dev [`35561028251`](https://github.com/sprmke/kame-homes/actions/runs/35561028251)). Full 21.6 table + Playwright still open.                        | Tests + seed data |
...
- [ ] Adversarial suite green; each case fails correctly when the check is removed. **Partial:** live smoke (**13** cases; last full cd-dev green **12/12** run `35561028251`).
- [ ] Permission expansion helpers unit-tested (21 prior art). **Partial:** `permissionExpansion_test.ts` (5) + UI `permissionExpansionDrift.test.ts` (4); ops/settings/org legacy expansions still open.
