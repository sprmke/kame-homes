...
| 7   | Adversarial suite (21.6 table). **Partial:** `adversarialAuthLive.test.ts` (**11 cases**, no seed) **11/11** on hosted dev (2026-09-21); **cd-dev** last **8/8** on [`35556921767`](https://github.com/sprmke/kame-homes/actions/runs/35556921767) until next `supabase/**` push; full 21.6 table + Playwright still open. | Tests + seed data |
...
- [ ] Adversarial suite green; each case fails correctly when the check is removed. **Partial:** live smoke (**11** cases in `adversarialAuthLive.test.ts`) **11/11** on hosted dev (2026-09-21); cd-dev **8/8** until next deploy.
