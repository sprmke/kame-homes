...
| 2   | Full restore drill on hosted-dev, including Storage; measure RTO vs target (30.2). **Partial:** runbook + `bun run rehearsal:rollback:dev:preflight` (2026-09-21: linked `fwor…`, local dev backup pair present; `rollback --dry-run` skipped without `DEV_DB_URL` in `supabase/.env.dev.local`). Measured drill still open. | Hosted-dev operator |
