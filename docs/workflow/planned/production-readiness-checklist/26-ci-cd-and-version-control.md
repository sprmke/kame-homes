...
| 8   | Zero quarantined-and-forgotten flaky tests. `guestFormSubmit` harness dates fixed (`c8d34bd`). **2026-09-21:** `marketingCollage.spec.ts` waits for collage mode via hidden helper copy + `Fill all` (cd-dev `35556921767` had a transient Photos timeout under parallel workers). Cross-check doc 29.                                   | Tests                         |
