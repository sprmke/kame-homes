...
| 4   | Snapshot `pg_stat_statements` top-50 on hosted-dev. **cd-dev step skipped** when `DEV_DB_URL` empty (latest: run `35556921767`, 2026-09-21). Add secret per `github-environments-setup.md` and re-run deploy; `gh run download <id> -n pg-stat-statements-hosted-dev`.                                                                                                                         | `DEV_DB_URL` in GitHub `development` |
