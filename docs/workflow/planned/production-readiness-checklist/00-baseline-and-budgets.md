...
| 3   | ~~Public edge p50/p95~~ **partial** — `baselines/2026-09-21-edge-latency-hosted-dev-public-v3.json` (4 public GETs, 12× each, 2026-09-21). **cd-dev** uploads `*-edge-latency-hosted-dev-ci.json` artifact after deploy. Authenticated endpoints still need `SUPABASE_ACCESS_TOKEN`.             | Token for auth endpoints             |
| 4   | Snapshot `pg_stat_statements` top-50 on hosted-dev. **cd-dev step ran** on runs `35544063765` and `35545944443` (2026-09-21) but **`DEV_DB_URL` GitHub secret empty** — no JSON committed; workflow warns on skip (`9299d7fb`). Add secret per `github-environments-setup.md` and re-run deploy. | `DEV_DB_URL` in GitHub `development` |
