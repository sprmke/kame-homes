...
| 3   | ~~Public edge p50/p95~~ **partial** — `baselines/2026-09-21-edge-latency-hosted-dev-public-v3.json` (12×) + `baselines/2026-09-21-edge-latency-hosted-dev-ci.json` from cd-dev [`35561028251`](https://github.com/sprmke/kame-homes/actions/runs/35561028251). Download: `gh run download 35561028251 -n edge-latency-hosted-dev`. Authenticated endpoints still need `SUPABASE_ACCESS_TOKEN`. | Token for auth endpoints             |
| 4   | Snapshot `pg_stat_statements` top-50 on hosted-dev. **cd-dev step skipped** when `DEV_DB_URL` empty (latest: run `35561028251`, 2026-09-21). Add secret per `github-environments-setup.md` and re-run deploy; `gh run download <id> -n pg-stat-statements-hosted-dev`.                                                                                                                         | `DEV_DB_URL` in GitHub `development` |
