...
| 3   | Doc 21 adversarial matrix executable and green. **Partial:** `adversarialAuthLive.test.ts` **15/15** on cd-dev [`35569769373`](https://github.com/sprmke/kame-homes/actions/runs/35569769373) (2026-09-21). Seeded matrix rows still open. | Doc 21 + tests           |
...
Inventory at the last audit: 25 UI unit tests, 0 component tests (by decision), 46 shared edge tests, 7 handler tests, 48 E2E specs — against 2330 UI files and 300 edge functions. **2026-09-21:** handler suite **51** Deno tests (`bun run test:edge:handlers`); live adversarial **15** on cd-dev `35569769373`.
