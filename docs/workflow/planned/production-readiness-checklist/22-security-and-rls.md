...
| 2   | RLS as `anon` + `authenticated`: **partial** — `probe-rls-anon-local.sh` (both roles, deny-by-default tables) in `migration-replay.yml` after `db:reset` (2026-09-21). Hosted `anon` probe via direct Postgres still open.                                             | Tests + hosted  |
