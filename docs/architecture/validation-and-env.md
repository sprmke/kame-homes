...
| `VITE_SENTRY_DSN`                 | No         | Browser **alerting** (`ui/src/lib/sentry/client.ts`), read at module scope. Unset or malformed → no-op. Called from `AppErrorBoundary` alongside `captureAppException`, deliberately independent of `VITE_POSTHOG_KEY` so either can run alone. Posts the envelope API directly (no `@sentry/browser`) to protect the initial-bundle budget. **Adding a non-`*.ingest.sentry.io` DSN host requires a matching `connect-src` entry in `ui/vercel.json`.** |
...
| `AI_RESPONSE_CACHE_DISABLED`                                                  | Optional. `1` bypasses the AI response cache (live evals set it). See `ai-platform.md`                                                                                                                                                                                                                                                         |
| `EDGE_INLINE_BACKGROUND_TASKS`                                                | Optional, local. `1` runs after-response work (inbox auto-reply) inline instead of `EdgeRuntime.waitUntil`                                                                                                                                                                                                                                     |
...
| `SENTRY_DSN`      | Server-side **alerting** (`_shared/sentry.ts`). Unset → no-op; a malformed DSN logs once and disables. Captured from the same `handleEdgeError` choke point as `POSTHOG_API_KEY`, settled in parallel with it. Added because PostHog alerting is operator-blocked (doc 28) — PostHog keeps exception history, Sentry pages. `extra` runs through the shared PII denylist (`posthogSanitize.ts`) |
