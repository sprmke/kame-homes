...
export async function handleEdgeError(
  ...
  // A deliberate EdgeError also returns its stable `code` so clients can branch on it
  // (e.g. `ai_verdict_blocked` → offer "Proceed anyway") without matching prose.
  if (error instanceof EdgeError && code) {
    return jsonResponse(
      req,
      {
        success: false,
