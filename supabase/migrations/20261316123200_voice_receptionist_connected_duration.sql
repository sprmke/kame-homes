-- Measure billable voice duration from provider setup and classify failed provider restarts.
...
BEGIN
  ...
  v_duration := CASE
    ...
    ELSE GREATEST(
      0,
      EXTRACT(
        EPOCH FROM (v_ended_at - COALESCE(v_session.connected_at, v_session.started_at))
      )::INT
    )
  END;
  ...
  SET
    status = CASE
      WHEN p_end_reason IN ('error', 'provider_error', 'provider_go_away') THEN 'failed'
